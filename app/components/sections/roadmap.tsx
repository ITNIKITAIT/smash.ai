  ...
  return (
    <section
      ...
      <div className="container px-4 mx-auto">
            ...
            {timeline.map((item, i) => (
              <motion.div
                ...
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-12 hidden lg:flex flex-col items-center">
                  <div
                    className={`w-4 h-4 rounded-full border-2 ${i === 0 ? 'bg-accent border-accent animate-pulse' : 'bg-background border-white/20 group-hover:border-primary transition-colors'}`}
                  />
                  <div className="h-8 w-0.5 bg-white/10 mt-2" />
                </div>

                <Card
                  className={`h-full border-white/10 transition-all duration-300 hover:border-primary/50 hover:bg-card/80 ${i === 0 ? 'bg-card' : 'bg-card/50'}`}>
                  <CardHeader>
                    <div className="flex justify-between items-center mb-2">
                      <span
                        className={`text-sm font-bold px-2 py-1 rounded-full ${i === 0 ? 'bg-accent/10 text-accent' : 'bg-white/5 text-muted-foreground'}`}>
                        {item.period}
                      </span>
                    </div>
        ...
        <div className="mt-16 p-8 rounded-2xl bg-gradient-to-r from-primary/10 via-secondary/10 to-accent/10 border border-white/10 text-center">
